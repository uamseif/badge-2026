...
uint8_t HAL_I2C_Transmit(I2C_TypeDef *i2c_periph, uint8_t DevAddress, uint8_t *pData, uint16_t Size, uint32_t i2c_timeout)
{
    ...
    while ((I2C_GetFlagStatus(i2c_periph, I2C_FLAG_BUSY) != RESET) && (HAL_get_tick() - tickstart < i2c_timeout))
        ;

    if (HAL_get_tick() - tickstart < i2c_timeout)
    {
    ...
    while (!I2C_CheckEvent(I2C1, I2C_EVENT_MASTER_MODE_SELECT) && (HAL_get_tick() - tickstart < i2c_timeout))
        ;
    ...
    while ((!I2C_CheckEvent(i2c_periph, I2C_EVENT_MASTER_TRANSMITTER_MODE_SELECTED)) && (HAL_get_tick() - tickstart < i2c_timeout))
        ;
    ...
    for (count = 0; count < Size; count++)
    {
        ...
        while ((!I2C_CheckEvent(i2c_periph, I2C_EVENT_MASTER_BYTE_TRANSMITTED)) && (HAL_get_tick() - tickstart < i2c_timeout))
            ;
...
uint8_t HAL_I2C_Receive(I2C_TypeDef *i2c_periph, uint8_t DevAddress, uint8_t *pData, uint16_t Size, uint32_t i2c_timeout)
{
    ...
    /* address flag set means i2c slave sends ACK */
    while ((!I2C_CheckEvent(i2c_periph, I2C_EVENT_MASTER_RECEIVER_MODE_SELECTED)) && (HAL_get_tick() - tickstart < i2c_timeout))
        ;
    ...
    for (count = 0; count < Size; count++)
    {
        ...
        while ((I2C_GetFlagStatus(i2c_periph, I2C_FLAG_RXNE) == RESET) && (HAL_get_tick() - tickstart < i2c_timeout))
            ;
